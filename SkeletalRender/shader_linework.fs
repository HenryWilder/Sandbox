...
for (int y_mult = -8; y_mult < 8; ++y_mult) {
    for (int x_mult = -8; x_mult < 8; ++x_mult) {
        edge += texture2D(texture0, vec2(fragTexCoord.x + (x_mult * x), fragTexCoord.y + (y_mult * y))).x;
    }
}
