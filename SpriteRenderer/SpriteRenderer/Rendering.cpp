...
void FrameBuffer::DrawSprite(PixelPos center, Sprite& sprite, WORD depth)
{
    Whole halfWidth = sprite.m_width / 2;

    for (Whole i = 0; i < sprite.m_size; ++i)
    {
        PixelPos pos{ (i % screen::width) - halfWidth, (i / screen::width) - halfWidth };

        DrawToBuffer(pos, sprite.m_colors[sprite.m_texture[i]]); // Draws the pixel from the frame buffer to the console
    }
}
...
void FrameBuffer::Draw()
{
    ...
    {
        Whole x = i % screen::width, y = i / screen::width;

        SetPixel(context, x, y, m_frame[i]); // Draws the pixel from the frame buffer to the console
    }
}
